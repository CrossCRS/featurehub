...
public class RegenerateEnvironmentTokenCommandTests
{
    ...
    [Fact]
    public async Task RegenerateToken_ShouldRegenerateToken_WhenEnvironmentExistsAndUserHasAccess()
    {
        var environments = new List<FeatureHub.Domain.Entities.Environment>
        {
            new() { Id = 1, ProjectId = 1, Name = "Environment 1", Token = "old-token-000000000000000000000042", IsActive = true, IsDeleted = false },
        };
        ...
        var handler = new RegenerateEnvironmentTokenCommandHandler(_mockContext.Object, _mockProjectAuthorization.Object);
        var command = new RegenerateEnvironmentTokenCommand("owner1", 1, 1);

        await handler.HandleAsync(command);

        Assert.NotEqual("old-token-000000000000000000000042", environments[0].Token);
        Assert.Equal(environments[0].Token, command.NewToken);
        _mockContext.Verify(c => c.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task RegenerateToken_ShouldThrowNotFoundException_WhenEnvironmentDoesNotExist()
    {
        var environments = new List<FeatureHub.Domain.Entities.Environment>();
        var mockDbSet = environments.BuildMockDbSet();
        _mockContext.Setup(c => c.Environments).Returns(mockDbSet.Object);
        ...
        var handler = new RegenerateEnvironmentTokenCommandHandler(_mockContext.Object, _mockProjectAuthorization.Object);
        var command = new RegenerateEnvironmentTokenCommand("owner1", 1, 999);

        await Assert.ThrowsAsync<NotFoundException>(() => handler.HandleAsync(command));
    }

    [Fact]
    public async Task RegenerateToken_ShouldThrowForbiddenAccessException_WhenUserHasNoAccess()
    {
        var environments = new List<FeatureHub.Domain.Entities.Environment>
        {
            new() { Id = 1, ProjectId = 1, Name = "Environment 1", Token = "old-token-000000000000000000000042", IsActive = true, IsDeleted = false },
        ...
        var handler = new RegenerateEnvironmentTokenCommandHandler(_mockContext.Object, _mockProjectAuthorization.Object);
        var command = new RegenerateEnvironmentTokenCommand("other-user", 1, 1);
